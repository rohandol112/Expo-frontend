...
import { z } from "zod";
import { X, ArrowLeft, ArrowRight, Send, Save, Calendar, Sparkles, Plus, Trash2, CheckCircle2, AlertCircle, Circle, Bold, Italic, Underline, List, Image as ImageIcon, Quote, Video, Play, type LucideIcon } from "lucide-react";
...
import { useAdminUserSearch, useAutoFillNewsTranslations, useNewsItem, useUpdateNews } from "@/hooks/api/useNews";
...
const STEPS = [
  { title: "Basic Information", description: "Edit news basic details" },
  { title: "Multi Language Content", description: "Edit content in multiple languages" },
...
>>>>>>> 49b00521fc221b60fba67cd66393986f29286f41
});
...
function EditNewsPage() {
  ...
  useEffect(() => {
    ...
    reset({
      ...
      subcategoryIds: news.categories?.slice(1).map((c) => String(c.id)) ?? [],
      ...
      status: news.status === "Scheduled" ? "schedule" : "draft",
      scheduledFor: news.scheduledFor ? new Date(news.scheduledFor).toISOString().substring(0, 16) : undefined,
      enablePoll: Boolean(news.poll),
      pollQuestion: news.poll?.question || "",
      pollOptions: news.poll?.options.map((option) => option.text) ?? ["Strongly Agree", "Agree", "Neutral", "Disagree", "Strongly Disagree"],
    });
  ...
  const translationStatus = (code: string) => {
    ...
    return draft.title && draft.description ? "Filled" : draft.title || draft.description ? "Partial" : "Empty";
  };
  ...
  const visibility = useMemo(() => {
    if (values.visibilityScope === "state") return { scope: "state" as const, state_ids: values.visibilityStateIds.map(Number) };
    if (values.visibilityScope === "district") return { scope: "district" as const, district_ids: values.visibilityDistrictIds.map(Number) };
    if (values.visibilityScope === "area") return { scope: "area" as const, area_ids: values.visibilityAreaIds.map(Number) };
    if (values.visibilityScope === "private") return { scope: "private" as const, user_ids: values.visibilityUserIds.map(Number) };
    return { scope: "all_india" as const };
  ...
  const buildPayload = (data: FormValues): UpdateAdminNewsPayload => {
  ...
  const submitNews = async (data: FormValues) => {
    try {
      ...
      try {
        ...
        await newsService.updateCategories(newsId, categoryPayload);
      } catch {
        // best-effort categories updates
      }
      ...
      if (thumbnailFile) {
        ...
        const upload = await newsService.requestThumbnailUploadUrl(newsId, {
        ...
        await newsService.confirmThumbnailUpload(newsId, { file_key: upload.file_key });
      }
      ...
      if (videoFile) {
        ...
        const upload = await newsService.requestVideoUploadUrl(newsId, {
        ...
        await newsService.confirmVideoUpload(newsId, { file_key: upload.file_key });
      }

      setUploadStatus(null);

      if (finalStatus === "schedule" && data.scheduledFor) {
        await newsService.schedule(newsId, new Date(data.scheduledFor).toISOString());
      }
      ...
      setUploadStatus(null);
      toast.error(isAuthApiError(err) ? "Backend admin auth is required to update news." : "Unable to update news or upload media");
    }
  };
  ...
  const goBack = () => (step === 0 ? navigate({ to: "/news/$newsId", params: { newsId } }) : setStep((current) => current - 1));

  if (newsQuery.isLoading) {
    return <div className="p-6 text-center text-muted-foreground">Loading news details...</div>;
  }
          ...
          <div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <MediaPicker
                ...
                icon={ImageIcon}
                accept="image/*"
                value={thumbnailFile || values.thumbnailUrl || null}
                onChange={(file) => {
                  setThumbnailFile(file);
                  if (!file) setValue("thumbnailUrl", "");
                }}
              />
              <MediaPicker
                ...
                icon={Video}
                accept="video/*"
                value={videoFile || news?.videoUrl || null}
        ...
        <div className="rounded-lg border bg-card">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b p-5">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Edit news content in multiple languages.</p>
            </div>
      ...
      <div className="flex items-center justify-between mt-6">
        ...
        <div className="flex gap-2">
          {step < 2 ? (
            ...
            <Button type="submit" disabled={updateNews.isPending}>
              <Send className="h-4 w-4 mr-1" /> {updateNews.isPending ? "Saving..." : "Save Changes"}
            </Button>
          )}
        </div>
      </div>
